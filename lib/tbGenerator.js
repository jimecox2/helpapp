...
// Risk picklists — off-list AI values are coerced so reports stay consistent
const RISK_PROBABILITY_VALUES = ['Very Unlikely', 'Unlikely', 'Likely', 'Very Likely', 'Certain']
const RISK_IMPACT_VALUES      = ['Very Low', 'Low', 'Medium', 'High', 'Very High']
const RISK_MITIGATION_STATUS_VALUES = [
  'Identified', 'Assessed', 'Mitigation Planned', 'In Progress', 'Mitigated',
  'Under Review', 'Accepted', 'Transferred', 'Escalated', 'Closed', 'Deferred',
]

function normalizePicklistValue(value, allowed, fallback) {
  const match = allowed.find(v => v.toLowerCase() === String(value || '').trim().toLowerCase())
...
=== tbID SCHEME (mandatory for every row) ===
- L1 Portfolio: "PF-100"
- L2 Project: "PJ001" (a second project would be "PJ002", and so on)
- L3 Sub-Project / Work Package: "SP100", incrementing by 1 in display order
- L4 Task / Milestone / Risk / Issue / CR: "T100", incrementing by 1 in display order
- L5 Allocation: "A100", incrementing by 1
Never invent any other tbID format. A TBID column in a CSV is reference data only — the output tbIDs always follow this scheme.

=== CSV HANDLING ===
A CSV document's first row contains column names matching tbTimebars/tbMetaData field names (match case-insensitively). Create one tbTimebars row per CSV data row (tbIDs per the tbID SCHEME, not the CSV's TBID column).
...
- High-level risks                 -> tbMDConsequence (general narrative only — individually listed risks become Risk rows, see RISK EXTRACTION)
...
=== RISK EXTRACTION ===
You are also an expert Risk Manager recording KNOWN risks. Risks can come from:
- a selected markdown document with "risk" in its filename — a standalone RISK LIST, and/or
- a risks section inside the charter/business case (headings, bullets, or a table).
EXTRACT and structure the risks the sources state — do NOT invent, add, merge or omit risks: one Risk row per stated risk, no more than 30.

DE-DUPLICATION (important): the same risk may appear in BOTH the standalone risk list and the charter. Create each distinct risk ONCE — when duplicated, keep the more detailed version (usually the standalone risk list) and fold in any extra detail from the other source. Never output two rows for the same underlying risk.

PLACEMENT: when there is at least one risk, create one L3 Sub-Project row named "Risk Register" under the L2 Project (tbType "Sub-Project", tbSelfKey2 = the Project's tbID, dates spanning the Project's dates), placed LAST among the Project's Sub-Projects. Every Risk row has tbType "Task", tbSubType "Risk", tbSelfKey2 = the Risk Register Sub-Project's tbID, tbWork 0. Dates: copy the Project's tbStart, tbFinish and tbDuration onto every Risk unless the source states a date for that risk (a review or closure date becomes tbFinish).

HOW TO READ THE SOURCE:
- Markdown: one risk per level-2/3 heading (e.g. "## Risk: Vendor delay"), per bullet point, or per table row — use whichever structure the text has.
- Plain text: one risk per numbered/dashed line or per paragraph.
- Everything belonging to one risk (description, probability, impact, mitigation, owner...) stays with that risk, whether written inline, as sub-bullets, or as table columns.

PER-RISK FIELDS — tbTimebars row: tbID per the tbID SCHEME (T-series, continuing the task numbering), tbName = concise risk title under 150 chars, tbOwner = the named risk owner when stated. On the risk's tbMetaData record (leave out when the source says nothing):
- tbMDNotes: the full risk statement as written
- tbMDProbability: EXACTLY one of: Very Unlikely, Unlikely, Likely, Very Likely, Certain — translate wording ("almost certain" -> Certain, "low chance" -> Unlikely). "Likelihood" in a risk register means Probability: map any Likelihood column, heading or value to tbMDProbability. "Not Assessed" if no likelihood is given.
- tbMDImpact: EXACTLY one of: Very Low, Low, Medium, High, Very High — translate wording ("severe", "critical" -> Very High). "Not Assessed" if not stated.
- tbMDConsequence: what happens if the risk materializes
- tbMDMitigationPlan / tbMDContingencyPlan: the stated mitigation and fallback plans
- tbMDRiskResponseStrategy: Avoid / Mitigate / Transfer / Accept etc. when stated
- tbMDTriggerEvent, tbMDEarlyWarningIndicators, tbMDEscalationLevel
- tbMDRiskCategory: stated category (e.g. Technical, Vendor, Schedule)
- tbMDMitigationStatus: EXACTLY one of: Identified, Assessed, Mitigation Planned, In Progress, Mitigated, Under Review, Accepted, Transferred, Escalated, Closed, Deferred. Default "Identified".
- Do NOT set tbMDScore — the system calculates it from probability and impact.
...
export function normalizeGeneratedData(raw) {
  ...
  let forcedTaskCount = 0
  bars.forEach(row => {
    row.canvasNo = 1
    DATE_FIELDS.forEach(f => { if (row[f] === undefined) row[f] = '' })
    if (!row.tbStart)  row.tbStart  = start
    if (!row.tbFinish) row.tbFinish = finish
    // §1.8: Risk / Issue / CR always ride on tbType "Task" (forced)
    const subType = normalizePicklistValue(row.tbSubType, ['Risk', 'Issue', 'CR'], '')
    if (subType) {
      row.tbSubType = subType
      if (row.tbType !== 'Task') { row.tbType = 'Task'; forcedTaskCount++ }
    }
  })
  if (forcedTaskCount > 0) notes.push(`Forced tbType "Task" on ${forcedTaskCount} Risk/Issue/CR row(s).`)
  ...
  // Hierarchical display order, enforced here rather than trusted from the
  // AI: depth-first from the L1 roots so every child sits directly below its
  // parent. tbCoordTop stacking depends on this order — a Sub-Project at 400
  // must be followed by its first child Task at 450.
  const childrenOf = new Map()
  const roots = []
  bars.forEach(row => {
    if (row.tbSelfKey2 === '') { roots.push(row); return }
    if (!childrenOf.has(row.tbSelfKey2)) childrenOf.set(row.tbSelfKey2, [])
    childrenOf.get(row.tbSelfKey2).push(row)
  })
  const ordered = []
  const visited = new Set()
  const visit = row => {
    if (visited.has(row.tbID)) return
    visited.add(row.tbID)
    ordered.push(row)
    for (const child of childrenOf.get(row.tbID) || []) visit(child)
  }
  roots.forEach(visit)
  if (ordered.length === bars.length) {
    bars = ordered
  } else {
    notes.push('Row order left as generated — the hierarchy contains a cycle or duplicate tbIDs.')
  }

  // Enforce the tbID scheme in hierarchy order, whatever the AI invented:
  // PF-100 (Portfolio), PJ001+ (Project), SP100+ (Sub-Project), T100+ (Task/
  // Milestone/Risk/Issue/CR), A100+ (Allocation). Original IDs are kept on
  // __origID so AI-provided tbMetaData records can still be matched, and all
  // tbSelfKey2 references are remapped.
  const counters = { PF: 100, PJ: 1, SP: 100, T: 100, A: 100 }
  const idMap = new Map()
  bars.forEach(row => {
    row.__origID = row.tbID
    let newId
    if (row.tbType === 'Portfolio')        newId = `PF-${counters.PF++}`
    else if (row.tbType === 'Project')     newId = `PJ${String(counters.PJ++).padStart(3, '0')}`
    else if (row.tbType === 'Sub-Project') newId = `SP${counters.SP++}`
    else if (row.tbType === 'Allocation')  newId = `A${counters.A++}`
    else                                   newId = `T${counters.T++}`
    idMap.set(row.__origID, newId)
    row.tbID = newId
  })
  bars.forEach(row => {
    if (row.tbSelfKey2 && idMap.has(row.tbSelfKey2)) row.tbSelfKey2 = idMap.get(row.tbSelfKey2)
  })

  // tbCoordTop stacking: L1 = 100, each row below adds 50, in hierarchy order
  bars.forEach((row, i) => {
  ...
  const tbMetaData = bars.map(bar => {
    const src = metaById.get(bar.__origID) || {}
    if (!metaById.has(bar.__origID)) createdMeta++
    ...
    // Risk rows: coerce picklist values; tbMDScore stays 0 (system-calculated)
    if (bar.tbSubType === 'Risk') {
      meta.tbMDProbability      = normalizePicklistValue(meta.tbMDProbability, RISK_PROBABILITY_VALUES, 'Not Assessed')
      meta.tbMDImpact           = normalizePicklistValue(meta.tbMDImpact, RISK_IMPACT_VALUES, 'Not Assessed')
      meta.tbMDMitigationStatus = normalizePicklistValue(meta.tbMDMitigationStatus, RISK_MITIGATION_STATUS_VALUES, 'Identified')
    }
    return meta
  })
  if (createdMeta > 0) notes.push(`Created ${createdMeta} missing tbMetaData record(s).`)

  bars.forEach(row => { delete row.__origID })
