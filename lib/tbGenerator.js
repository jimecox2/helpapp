...
=== tbMetaData ===
One record per tbTimebars row (strict one-to-one). tbMDID must exactly equal the row's tbID. Include tbMDID plus only meaningful fields (e.g. tbMDNotes, tbMDWBS, tbMDPhase from the data, a one-sentence tbMDDescription-style note in tbMDNotes if the documents support it). Defaults (tbMDStatus "New", tbMDHealth "Not Assessed", mirrors of tbName/tbType) are applied automatically.
