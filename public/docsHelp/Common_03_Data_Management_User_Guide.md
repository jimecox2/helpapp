# Data Management User Guide - Common Across Products
...
- **What it stores:** All dropdown list values (picklists) used throughout the application, plus a set of internal system configuration entries
...
> **Warning — Protected System Entries:** The Tags store contains two categories of data. The first is ordinary user-facing picklist values (safe to add, edit, or delete). The second is a set of **system configuration entries** that the dynamic forms engine (FOCD) requires to render forms at runtime. These system entries are identified by having a `tbTagGroup` value of exactly: **"Tag Group"**, **"Form Name"**, **"Field Name"**, **"Field Type"**, or **"Field State"**. Do not delete, rename, or alter any tag row belonging to these five groups. Doing so will cause dynamic forms to stop rendering, fields to disappear, or the tag management interface to break. They look identical to ordinary tag rows in both the application grid and the spreadsheet — use the `tbTagGroup` column to identify and avoid them. See the *FOCD Forms User Guide* for full details.
...
- **Tags worksheet — protected rows:** The Tags worksheet includes system configuration entries alongside your regular picklist values. Any row where the `tbTagGroup` column reads "Tag Group", "Form Name", "Field Name", "Field Type", or "Field State" is a protected system entry. Do not delete, rename, or modify these rows. They are required by the FOCD dynamic forms engine and cannot be reconstructed without restoring from a backup.
...
- **When cleaning up tags:** Filter the Tags grid or Tags worksheet by `tbTagGroup` before deleting stale entries. Never delete rows where `tbTagGroup` is "Tag Group", "Form Name", "Field Name", "Field Type", or "Field State" — these are system configuration entries, not user data. If accidentally deleted, restore from a backup taken before the deletion.
